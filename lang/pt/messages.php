<?php

return [
    'navbar' => [
        'contact' => 'CONTACTE-NOS',
        'competion' => 'Competição',
        'training' => 'Treinos',
        'eventos' => 'Eventos',
        'wjjc' => 'WJJC',
        'gallery' => 'GALERIA',
        'dojos' => 'DOJOS',
        'diretor_tec' => 'DIRETOR TÉCNICO',
        'ju-jitsu' => 'KISHIDÕ JU-JITSU',
        'founder' => 'O FUNDADOR',
        'logo' => 'O logo S. George',
        'certification' => 'A certificação da WJJC',
        'history_wjjc' => 'A história da WJJC',
        'sponsors' => 'Patrocinadores',
        'affiliations' => 'FIliações',
        'association' => 'A Associação',
    ],
    'association' => [
        'association' => 'A Associação',
        'world' => 'A associação WJJC Portugal é uma organização sem fins lucrativos fundada em 2024 e cuja missão é a disseminação e
                                desenvolvimento do Kishidō Ju-Jitsu pelo território nacional.',
    ],
    'affiliations' => [
        'filition' => 'Filiações',
        'federation' => 'Federação Ju-Jitsu e Disciplinas Associadas Portugal',
        'national' => 'Nacional',
    ],
    'sponsors' => [
        'construction' => 'Em Construção',
        'textbig' => 'Estamos a atualizar as informações sobre os nossos patrocinadores. Agradecemos a sua paciência e
                            em breve teremos novidades para partilhar. Enquanto isso, sinta-se à vontade para voltar à
                            nossa página inicial.',
    ],
    'description' => 'WJJC PORTUGAL',
    'keywords' => 'wjjc, wjjcportugal, wjjcinternational, ju-jitsu',
    'title' => 'WJJC Portugal',
    'footer' => [
        'affiliation' => 'FILIAÇÃO',
        'contact_us' => 'CONTACTE-NOS',
        'email' => 'Email :',
        'wjjc_portugal' => 'WJJC PORTUGAL',
        'wjjc_international' => 'WJJC INTERNACIONAL',
        'website' => 'Website:'
    ],
    'index' => [
        'allgreat' => 'TODAS AS GRANDES',
        'journeys' => 'JORNADAS COMEÇAM COM UM ÚNICO PASSO',
        'joinus' => 'Junte-se a nós',
        'international' => 'WJJC Internacional',
        'latest' => 'ÚLTIMAS ',
        'news' => 'NOTÍCIAS',
        'welcome' => 'Boas Vindas a Portugal na WJJC',
        'worldjujitsu' => ' A World Ju-Jitsu Corporation tem o orgulho de anunciar que o Sensei Pedro Nuno Pimentel
                                    tornou-se o Diretor Técnico Nacional da WJJC Portugal 🇵🇹. Toda a Wjjc deseja ao Sensei
                                    Pimentel um bom trabalho e muito sucesso no seu país. Bem-vindo à nossa
                                    grande família. Oss!',
        'may' => 'Maio',
        'july' => 'Julho',
        'seminary' => 'Estágio Interno Ju-Jitsu Penela Julho 2024',
        'exclusive' => 'Estágio exclusivo com o Soke Adriano Busà e Sensei Pedro Nuno Pimentel',
        'masterclass' => 'Master Class 2024 Florença - Itália',
        'blackbelts' => 'A World Ju-Jitsu Corporation está a organizar uma Master Class Internacional aberta a
                                    todos os Diretores Técnicos Nacionais oficiais da WJJC e aos seus cintos negros.'
    ],
    'certification' => [
        'certification' => 'Certificação',
        'wjjc_certification' => 'A Certificação da ',
        'september' => 'Em setembro de 2015, Soke Adriano Busà levou consigo uma delegação de cintos negros
                                    da World Ju-Jitsu Corporation a Bruxelas, na Comissão Europeia, para integrar o curso do
                                    Quadro Europeu de Qualificação, ministrado pelo Prof. Giovanni Gordiani, presidente da
                                    EurEthics (Desporto para Todos). ',
        'course' => 'O curso teve como objetivo permitir à World Ju-Jitsu
                                    Corporation obter uma licença exclusiva para formar profissionais nos sectores do
                                    Ju-Jitsu e
                                    da defesa pessoal profissional que permite à WJJC a emissão de diplomas de
                                    treinador desportivo no âmbito da certificação ISO 17024 que disciplina as competências
                                    pessoais, sendo esta certificação necessária para a inscrição no Registo Europeu de
                                    Profissionais.',
    ],
    'logo' => [
        'logo_title' => 'O LOGO:',
        'logo_title2' => 'São Jorge',
        'logo_s' => 'O símbolo do WJJC é São Jorge matando o dragão com sua lança (Capadócia, cerca de 275 – 285
                                dC - Nicomédia 303). São Jorge foi o bravo cavaleiro que desafiou o horrível dragão e, no
                                final, o perfurou com sua lança. O Santo está associado à honra, bravura, bravura e
                                generosidade.
                                George era um cavaleiro brilhante e destemido que viveu no século III dC na Capadócia, uma
                                área que hoje faz parte da Turquia. Sua bravura foi demonstrada ajudando os outros.',
        'history' => 'Uma dessas histórias é assim...... naquela época, um dragão feroz vivia em um grande lago. O
                                dragão estava aterrorizando as cidades vizinhas e, todos os dias, as pessoas alimentavam o
                                dragão com uma ovelha para apaziguá-lo. Quando todas as ovelhas foram comidas, o dragão
                                exigiu que o povo sacrificasse uma jovem donzela a ele por dia. Uma por uma, as meninas
                                foram mortas até que, no final, sobrou apenas a filha do rei do Egito. A menos que um
                                cavaleiro fosse encontrado para matar o dragão, a princesa seria sacrificada no dia
                                seguinte.',
        'king' => 'O rei tentou negociar uma saída, mas o povo da cidade declarou que ela deveria ser entregue
                                ao dragão, assim como muitos de seus filhos haviam sido. Então, o rei vestiu sua filha como
                                se ela fosse se casar, abraçou-a, beijou-a, deu-lhe sua bênção e conduziu-a até onde estava
                                o dragão.',
        'george' => 'George, que estava passando, perguntou à filha o que estava acontecendo. Ela contou a ele
                                sobre o dragão e implorou que ele fosse embora antes que ele aparecesse e o matasse também.
                                O cavaleiro disse à princesa: “Linda filha, não duvide, eu te ajudarei. Bom cavaleiro, siga
                                seu caminho e não fique comigo, porque você não pode me livrar”.',
        'speak' => 'Enquanto eles conversavam, o dragão apareceu e avançou sobre eles. O dragão era enorme e seu
                                rugido parecia um trovão, mas São Jorge não teve medo. Ele montou seu cavalo, sacou a espada
                                e cavalgou com força contra o dragão. Mas o dragão atacou-o, feriu-o e feriu-o e atirou-o ao
                                chão.',
        'sao' => 'São Jorge não desistiu. Em vez disso, ele atacou o dragão e usou sua espada para perfurá-lo
                                sob suas asas, onde o dragão era vulnerável porque não havia escamas. O dragão caiu morto
                                sob os pés de São Jorge.
                                A filha do rei do Egito foi resgatada.',
        '302' => 'George estava igualmente destemido no dia de seu cruel martírio. No ano 302, Diocleciano
                                emitiu um decreto determinando que todos os soldados do exército deveriam oferecer
                                sacrifícios aos deuses romanos. Por se recusar a queimar incenso aos deuses, o imperador
                                mandou prendê-lo. Diocleciano, não querendo perder um de seus melhores tribunos, tentou
                                oferecer a Jorge presentes como terras, dinheiro e escravos se ele fizesse sacrifícios aos
                                deuses romanos. George nunca aceitou a oferta. O imperador o executou por sua recusa em
                                obedecer ao decreto. Antes da execução, George deu sua riqueza aos pobres e preparou-se para
                                morrer.',
        'big' => 'O próprio George não apenas não se submeteu às ordens, mas também deu grande coragem aos
                                outros prisioneiros com seu exemplo.',
    ],

    'dtn' => [
        'dtnnc' => 'Diretor Técnico Nacional',
        'jujitsu' => 'Sensei Pedro Nuno Pimentel, natural de Coimbra foi o escolhido pelo fundador da World
                                Ju-Jitsu
                                Corporation, Soke Adriano Busà, para estar à frente da WJJC ​Portugal na condição de Diretor
                                Técnico
                                Nacional e representante português desta organização internacional. Tendo como inicio a sua
                                caminhada
                                marcial em 1997 na ​modalidade do Kick Boxing com o Mestre Nuno Ferreira no KickGym de
                                Coimbra, em 2001
                                teve o seu primeiro contacto com esta arte conhecida como arte gentil, o ​“Ju-Jitsu” através
                                da
                                Associação Portuguesa de Nihon Ju-Jitsu e Defesa Pessoal pela mão do Mestre Hélder Nunes
                                onde
                                rapidamente se apaixonou pela Arte e se ​manteve ligado até hoje. Em 2015 iniciou
                                paralelamente à sua
                                caminhada no Nihon Ju-Jutsu, o seu trajecto no Judo com o Mestre Reinaldo Aquilino do
                                C.A.M.A, tendo ​no
                                ano de 2024 ingressado no curso de treinador de Judo promovido pela ADJA e Federação
                                Portuguesa de Judo.',
        'martial' => 'A sua incansável paixão pelas artes marcais levou-o à busca pelo conhecimento mais
                                aprofundado nas
                                disciplinas marciais dos antigos Samurais e foi em busca de ​aprender a forma de utilização
                                do sabre
                                japonês, a Katana, praticando também o manejo do sabre através da Arte do Mugai ryu Iaido e
                                Hachiman ryu
                                Battojutsu, ​sendo este segundo um estilo originário da família de Hamamoto Hisao Sensei,
                                oriundo da
                                Ilha de Okinawa, Japão; tendo como seu professor direto em Portugal o​ Sensei Jon Berry, a
                                nível
                                internacional e também presidente da W.T.M.A.S o Soke Leif Hermansson da Suécia.',
        'sensei' => 'Sobre o Sensei',
        '2014' => 'Desde 2014 que é o treinador e responsável técnico do Nihon Seishin Dojo em Penela, onde continua a
                    desenvolver a sua paixão pelas artes marcais, estando ​atualmente ligado ás disciplinas do Kishido
                    Ju-Jitsu da World Ju-Jitsu Corporation (Itália), o Kyoo Soku - Defesa Pessoal da Kyoo Soku Portugal, o
                    Kodokan Judo​ da FPJ e Mugairyu Iaido/Hachiman ryu Battojutsu da Ken Shin Kan Portugal',
        'objective' => 'O seu grande objectivo além da pratica marcial e desportiva é conseguir despertar, em todos os que o
                    procuram, a paixão. o interesse no estudo e prática destas ​disciplinas conseguindo assim uma
                    transmissão de valores éticos que influenciem positivamente as suas vidas independentemente da sua
                    idade, género e/ou raça.
',
    ],

    'wjjc' => [
        'wjjc-internacional' => 'WJJC Internacional',
        'history' => 'A História da ',
        'world' => 'A World Ju-Jitsu Corporation é resultado de seu carismático fundador, Shodai Soke Adriano
                                Busà, e sua longa experiência.',
        'organization' => 'Esta organização é uma jovem corporação preocupada em estudar e propagar esta doce arte
                                entre os cinco continentes. Disponibilizamos a nós próprios, e os nossos métodos modernos e
                                eficazes, a qualquer pessoa que deseje aprofundar o seu conhecimento de Ju-Jitsu. ',
        'english' => 'O estilo da WJJC está constantemente a ser atualizado. Estudamos continuamente as dinâmicas
                                urbanas internacionais para preparar os nossos alunos para enfrentar um ambiente em
                                constante mudança, sem abdicar das raízes tradicionais do Ju-Jitsu inglês.',
        'training' => 'faz parte de um caminho de treino de alto nível ensinado utilizando os valores típicos das
                                artes marciais
                                tais como: honra, respeito, disciplina, sacrifício e lealdade.',
        'tradicional' => 'Raízes tradicionais do Ju-Jitsu Inglês',
        'studing' => 'O estudo aprofundado de técnicas de defesa contra facas e armas',
        'tecnics' => 'Técnicas de defesa contra múltiplos ataques e grappling',
        'program' => 'O programa profissional, foi desenvolvido para Militares e Polícia',
        'fund' => 'Fundador',
        'strong' => 'APRIMORAMENTO DA FORÇA MUSCULAR:',
        'strong-p' => 'O treino envolve muitos grupos musculares, promovendo o fortalecimento do corpo inteiro.',
        'geral' => 'CONDICIONAMENTO FÍSICO GERAL',
        'geral-p' => 'A prática regular melhora a força, a flexibilidade, a resistência cardiovascular e a
                                    coordenação motora',
        'etics' => 'INTENSIFICAÇÃO DE VALORES ÉTICOS E MORAIS',
        'etics-p' => 'Princípios como honra, coragem, solidariedade e justiça são ensinados para ajudar na
                                    formação do caráter dos praticantes, especialmente crianças, adolescentes e jovens.',
        'continent' => 'Continentes',
        'country' => 'Países',
        'friends' => 'Membros Amigos',
    ],
    'wjjcpt' => [
        'wjjc-internacional' => '>WJJC Portugal',
        'we' => 'A nossa',
        'history' => 'História',
        'world' => 'A WJJC Portugal é uma organização sem fins lucrativos e cuja missão é a disseminação e
                                desenvolvimento do Kishidō Ju-Jitsu pelo território nacional.',
        'organization' => 'A nossa filosofia é
                                que a complementaridade será sempre uma poderosa ferramenta de crescimento e no qual através
                                da partilha de conhecimento, todos ganhamos porque todos
                                crescemos.',
        'english' => 'Se tiveres vontade ou curiosidade, sejas um clube ou a solo serás sempre bem vindo na WJJC
                                Portugal, estaremos cá para não só para te transmitir o Kishidō Ju-Jitsu,mas também para te
                                apoiar e para aprender contigo. Se procuras uma organização profissional de Ju-Jitsu a World
                                Ju-Jitsu Corporation é a indicada para ti!',
        'training' => 'Vem treinar connosco... o Kishidō Ju-Jitsu espera por ti! OSS “',
        'tradicional' => 'Raízes tradicionais do Ju-Jitsu Inglês',
        'studing' => 'O estudo aprofundado de técnicas de defesa contra facas e armas',
        'tecnics' => 'Técnicas de defesa contra múltiplos ataques e grappling',
        'program' => 'O programa profissional, foi desenvolvido para Militares e Polícia',
        'strong' => 'APRIMORAMENTO DA FORÇA MUSCULAR:',
        'strong-p' => 'O treino envolve muitos grupos musculares, promovendo o fortalecimento do corpo inteiro.',
        'geral' => 'CONDICIONAMENTO FÍSICO GERAL',
        'geral-p' => 'A prática regular melhora a força, a flexibilidade, a resistência cardiovascular e a
                                    coordenação motora',
        'etics' => 'INTENSIFICAÇÃO DE VALORES ÉTICOS E MORAIS',
        'etics-p' => 'Princípios como honra, coragem, solidariedade e justiça são ensinados para ajudar na
                                    formação do caráter dos praticantes, especialmente crianças, adolescentes e jovens.',
    ],
    'founder' => [
        'founder' => 'O Fundador',
        'jujitsu' => 'Shodai Soke Adriano Busà 9º Dan é o fundador e diretor técnico internacional da World
                                Ju-Jitsu Corporation. O estilo ​de Ju-Jitsu que ele codificou é o “Kishidō” - 騎士 道 (O
                                caminho do Cavaleiro).
                                Mais tarde fundou a ANS S.r.l (Agência Nacional de Segurança), uma empresa dedicada à
                                formação profissional para polícias e militares.
                                Além de levar adiante a sua ​grande paixão pelo Ju-Jitsu, esta agência é a única empresa em
                                Itália com certificação de qualidade UNI EN ISO 9001 e ​creditação da Região da Toscana.
',
        '2014' => 'Em 2014, fundou a World Ju-Jitsu Corporation em Londres e começou assim uma nova fase na sua
                                vida e na sua ​carreira marcial, sempre em ascensão.
',
        'filosofi' => 'A sua visão filosófica levou-o a organizar a World Ju-Jitsu Corporation de tal forma que a
                                todos é dada a oportunidade ​de crescer e melhorar, ao mesmo tempo que ajuda os outros a
                                fazer o mesmo, uma vez que seguem os mesmos ​princípios. A sua filosofia está contida no
                                famoso lema dos três mosqueteiros de Alexandre Dumas: “Um por todos e todos por Um!”
                                convencido da importância de forjar um grupo e fazer com que todos os membros do grupo se
                                sintam ​orgulhosos.',
        'soke' => 'Sobre o Soke',
        'sokeadri' => 'O Soke Adriano Busà é um verdadeiro líder, que nunca desiste, incansável, sempre pronto para a ação, não
                    só no tatami. ​Não só está sempre disponível para os seus alunos directos, mas também para todos os
                    alunos da WJJC, especialmente ​para os mais jovens, de facto, tem um sentimento de simpatia muito forte
                    por eles. É um ponto de referência para muitos ​deles, pronto a ouvi-los, estabelece uma relação
                    profunda baseada no respeito mútuo e é bom a dar conselhos e ideias ​tanto marciais como pessoais.
',
        'tecnics' => 'As competências técnicas do Soke Adriano Busà são muito apreciadas em todo o mundo e permitiram-lhe
                    reinterpretar o ​Ju-Jitsu tradicional e transformar em “Kishidō” o estilo da WJJC, um estilo
                    simultaneamente moderno e eficaz, baseado ​na precisão e na rapidez, embora permanecendo firmemente
                    enraizado nos valores tradicionais do Budo.
',
        'day' => 'Hoje em dia, o Soke Adriano Busà, com o seu carisma, continua a transmitir o seu estilo de Ju-Jitsu a
                    todos os que ​desejam juntar-se à World Ju-Jitsu Corporation e que partilhem os seus valores, dando-lhes
                    a oportunidade de se ​tornarem verdadeiros profissionais do Ju-Jitsu e de fazerem parte de uma grande
                    organização internacional inovadora, ​moderna e bem sucedida.
',
    ],

    'dojos' => [
        'dojo' => 'Os Dojos da',
        'wjjc' => 'WJJC Portugal',
        'follow' => 'Segue-nos: ',
        'email' => 'Enviar E-mail',
    ],

    'events' => [
        'event' => 'Eventos',
        'gallery' => 'Galeria de',
        'all' => 'Todos',
        'master' => 'Master Class Itália 2024',
        'seminar' => 'Estágio Interno Ju-Jitsu Penela Julho 2024',
    ],

    'workout' => [
        'gallery' => 'Galeria',
        'gallery_the' => 'Galeria de',
        'training' => 'Treinos',
        'all' => 'Todos',

    ],

    'contacts' => [
        'contact' => 'Contacte-nos',
        'done' => 'Pronto para obter mais',
        'information' => ' informação ',
        'email' => 'Enviar E-mail',
        'art' => 'A arte suave é sobre suavidade e flexibilidade, não força bruta.',
        'address' => 'Morada Sede',
        'watchmap' => 'ver mapa',
        'mail' => 'Enviar mail',
        'sendnow' => 'enviar agora',
        'question' => 'Qualquer pergunta? Sinta-se livre para ',
        'contactus' => 'Contactar-nos',
        'join' => 'Junta-te a',
        'we' => 'nós',
        'follow' => 'Segue-nos: ',
        'informations' => 'Para mais informações contacte pelo seguinte email:',

    ],

    'kishido' => [
        'cav' => 'O caminho do cavaleiro é a tradução literal da palavra japonesa Kishidō, o estilo da WJJC
                                codificado pelo Soke Adriano Busà. ',
        'inglesa' => 'Kishidō é o resultado de quatro décadas de estudo das artes marciais pelo nosso fundador. O
                                estilo da WJJC tem raízes ​inglesas, mas foi desenvolvido em Itália após longas experiências
                                profissionais do seu fundador, tanto no seu país como em todo ​o mundo.',
        'movements' => 'Embora o Kishidō esteja ancorado nos movimentos e na força do método inglês, é uma evolução
                                do mesmo e as suas técnicas ​estão mais centradas na velocidade e na precisão, o que melhora
                                o programa de treino e torna o nosso estilo ainda mais eficaz.

                                Acreditamos que as artes marciais devem trazer ao de cima o melhor do seu praticante e ser
                                aplicáveis a contextos de rua ​quando necessário. Por isso, não conseguirá atingir os seus
                                objectivos se não desenvolver velocidade, precisão e potência.

                                É isto que São Jorge, o coração pulsante do nosso logótipo representa. Ele derrotou o dragão
                                através da sua velocidade e ​bravura e é exatamente isso que procuramos com a prática do
                                Kishidō, formar pessoas conscientes das suas potencialidades e ​capazes de as pôr em prática
                                quando necessário.',
        'work' => ' Outro aspeto essencial é o desenvolvimento da coragem, que exige trabalhar primeiro através
                                de si próprio e depois enfrentar o ​adversário, utilizando a sua força e os seus movimentos
                                em sua desvantagem para virar a sua agressividade de pernas para o ar.

                                Este é o princípio universal do Ju-Jitsu, trabalhar sobre nós próprios, trabalhar com o
                                nosso corpo para chegar à mente, onde se ​encontram todas as nossas fraquezas, de modo a
                                transformá-las em forças.',
        'we' => 'PRINCIPAIS BENEFÍCIOS DO NOSSO',
        'aprimora' => 'Aprimoramento da Força Muscular:',
        'musculs' => 'O treino envolve muitos grupos musculares, promovendo o fortalecimento do corpo inteiro.',
        'geral' => 'Condicionamento Físico Geral',
        'strong' => 'A prática regular melhora a força, a flexibilidade, a resistência cardiovascular e a coordenação motora',
        'weigh' => 'Perda de Peso',
        'help' => 'As sessões de treino intensas ajudam a queimar calorias e a reduzir a gordura corporal. ',
        'value' => 'intensificação de valores
                                                                éticos e morais',
        'pride' => 'Princípios como honra, coragem, solidariedade e justiça fazem
                                                                parte dos ensinamentos, ajudando a formar o caráter dos
                                                                praticantes, principalmente no caso de crianças,
                                                                adolescentes e jovens',
        'autocontrol' => 'Autoconfiança e Autocontrole',
        'ju' => 'A prática regular ajuda a aumentar a autoconfiança, pois os
                                                                praticantes aprendem a lidar com desafios e adversidades.
                                                                Além disso, o ju-jutsu ensina autocontrole emocional e
                                                                mental, ajudando a lidar com situações estressantes de
                                                                maneira mais calma e eficaz.',
        'defesa' => 'DEFESA PESSOAL EFICAZ',
        'tecnic' => 'O Kishido Ju-Jitsu ensina técnicas práticas de defesa pessoal
                                                                que podem ser aplicadas em situações reais de conflito,
                                                                ajudando os praticantes a se defenderem eficientemente
                                                                contra agressões.',
        'this' => 'Para quem é esta ',
        'martial' => 'Arte Marcial?',
        'benefic' => 'O Ju-Jitsu é uma arte marcial inclusiva que oferece benefícios a uma ampla variedade
                                        de pessoas',
        'training' => 'o Ju-Jitsu proporciona um treino completo que melhora a força, a resistência e a
                                        flexibilidade, contribuindo para um condicionamento físico geral. A prática regular
                                        também ajuda a aliviar o stress e a ansiedade, promovendo um bem-estar mental
                                        equilibrado. Além disso, os adultos aprendem técnicas eficazes de autodefesa,
                                        aumentando a sua confiança em situações do dia-a-dia.',

    ],


    // Adicione todas as outras traduções necessárias
];
